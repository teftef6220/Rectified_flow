    ...
    def start_epoch(self, world_size=None, global_rank=None):
        ...
        for bucket_id in sorted(self.buckets.keys()):
            if len(self.buckets[bucket_id]) > 0:
                self.epoch[bucket_id] = np.array([post_id for post_id in self.buckets[bucket_id] if post_id in index], dtype=object)
...
if __name__ == "__main__":
    # prepare a pickle with mapping of dataset IDs to resolutions called ./make_dataset/resolutions.pkl to use this
    with open("./make_dataset/resolutions.pkl", "rb") as fh:
    ...
    bm = BucketManager("./make_dataset/resolutions.pkl", debug=True, bsz=8, world_size=8, global_rank=3)
    ...
    bm = BucketManager("./make_dataset/resolutions.pkl", bsz=8, world_size=1, global_rank=0, valid_ids=ids[0:16])
    for _ in range(16):
        bm.get_batch()
    ...
    for rank in range(16):
        bm = BucketManager("./make_dataset/resolutions.pkl", bsz=8, world_size=16, global_rank=rank)
        bms.append(bm)
